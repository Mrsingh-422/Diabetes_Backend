...
// ==========================================
const scienceDir = 'public/uploads/science';
ensureDir(scienceDir);

const scienceUploads = multer({
    storage: multer.diskStorage({
        destination: (req, file, cb) => cb(null, scienceDir),
        filename: (req, file, cb) => cb(null, `science-${Date.now()}${path.extname(file.originalname)}`)
    }),
    fileFilter: docFileFilter,
...
// ==========================================
// 48. VIDEO CONFIGURATION (Video + Thumbnail)
// ==========================================
const videoDir = 'uploads/admin/user/video';
const thumbnailDir = 'uploads/admin/user/thumbnail';
ensureDir(videoDir);
...
// 48a. CREATE VIDEO (video1-6 + thumbnail1-6)
...
// 48b. UPDATE VIDEO (video1-6 only, no thumbnail)
const videoUpdateUploads = multer({
    storage: multer.diskStorage({
...
// ==========================================
// 49. FOOTER CONTENT IMAGES CONFIGURATION
// ==========================================
const footerDir = 'public/uploads/user/footer';
ensureDir(footerDir);

const footerUploads = multer({
    storage: multer.diskStorage({
        destination: (req, file, cb) => cb(null, footerDir),
        filename: (req, file, cb) => cb(null, `footer-${Date.now()}${path.extname(file.originalname)}`)
    }),
    fileFilter: docFileFilter,
    limits: { fileSize: 10 * 1024 * 1024 } // 10MB Limit
}).fields([
...
// ==========================================
// 50. ABOUT US IMAGES CONFIGURATION
// ==========================================
const aboutUsDir = 'public/uploads/aboutus';
ensureDir(aboutUsDir);

const aboutUsUploads = multer({
    storage: multer.diskStorage({
        destination: (req, file, cb) => cb(null, aboutUsDir),
    ...
    { name: 'insuranceLogos', maxCount: 10 },
