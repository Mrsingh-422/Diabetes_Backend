...
const FoodBooking = require('../../../models/FoodBooking');       // 👈 Fixed Import
...
const calculateProviderBalances = async (vendorId, role) => {
    ...
    if (role === 'Lab') {
        ...
        BookingModel = FoodBooking; // 👈 Resolved to correct model
        matchQuery = { FoodId: new mongoose.Types.ObjectId(vendorId) };
    ...
    const totalWithdrawalsQuery = await WithdrawalRequest.aggregate([
        {
            $match: {
                ...
                vendorModel: role, // Dynamically maps to Lab, Pharmacy, or Food
...
const getWalletStats = async (req, res) => {
    try {
        ...
        const role = req.user.role; // 'Lab', 'Pharmacy', or 'Food'
...
const updateProviderBankDetails = async (req, res) => {
    try {
        ...
        else if (role === 'Food') VendorModel = Food;
