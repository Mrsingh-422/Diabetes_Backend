...
// ==========================================
// ⏸️ SKIP TIFFIN MEALS (NO DATE EXTENSION)
// Full Path: PATCH /api/food/tiffin/skip-meals/:bookingId
// ==========================================
const skipTiffinMeals = async (req, res) => {
    try {
        ...
        // 🚨 Date extension removed: Original endDate remains strictly unchanged
        if (!booking.skippedMeals) {
            booking.skippedMeals = [];
        }
        booking.skippedMeals.push(skipEntry);
        ...
        const planEndDate = booking.bookingType === 'Subscription' 
            ? booking.subscriptionDetails?.endDate 
            : booking.customTiffinDetails?.endDate;

        return res.json({
            success: true,
            message: `Meals skipped successfully from ${startDate} to ${endDate} (${skippedDaysCount} days).`,
            data: {
                bookingId: booking.bookingId,
                bookingType: booking.bookingType,
                status: booking.status,
                ...
                planEndDate: planEndDate ? planEndDate.toISOString().split('T')[0] : null,
                updatedAt: booking.updatedAt
            }
        });
