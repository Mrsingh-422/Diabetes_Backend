...
const getLocalDatabaseContext = async (message, userId) => {
    ...
    const words = text.split(/\s+/).filter(w => w.length > 2 && !['list', 'show', 'find', 'search', 'give', 'some', 'any', 'the', 'for', 'with', 'please',  'doctor', 'medicine', 'lab', 'test',  'Food', 'of', 'in', 'near', 'price', 'rate', 'cost', 'track', 'my', 'status', 'review', 'rating', 'star', 'feedback', 'comment'].includes(w));
    ...
    if (text.includes('my') || text.includes('track') || text.includes('order') || text.includes('booking') || text.includes('appointment') || text.includes('status')) {
        ...
        const [appointment, lab,  Food] = await Promise.all([
            ...
            FoodBooking.findOne({ userId }).populate('FoodId', 'name').sort({ createdAt: -1 }).lean()
        ]);
        ...
        if (Food) {
            hasBookings = true;
            context += `- Food: ID ${Food.bookingId || 'N/A'}, Status: ${Food.status}, Food: ${Food.FoodId?.name}\n`;
        }
        if (!hasBookings) {
    ...
    // 6. Food Services
    else if (text.includes('Food') || text.includes('elderly care') || text.includes('nursing') || text.includes('care service') || text.includes('home care')) {
        context += `\n[DB_FoodS]:\n`;
        const FoodQuery = { isActive: true, profileStatus: 'Approved' };
        if (targetLocation) FoodQuery.city = new RegExp(targetLocation, 'i');

        const Foods = await Food.find(FoodQuery).select('name city speciality').limit(2).lean();

        if (Foods.length > 0) {
            const FoodIds = Foods.map(n => n._id);
            const services = await FoodService.find({ FoodId: { $in: FoodIds }, status: 'Approved' }).select('title pricing FoodId').limit(2).lean();

            Foods.forEach(Food => {
                const sList = services.filter(s => s.FoodId.toString() === Food._id.toString());
                context += `- Food: ${Food.name} (${Food.city})\n` + (sList.length > 0 ? `  * Services: ` + sList.map(s => `${s.title} (₹${s.pricing?.oneDay?.final || 0})`).join(', ') : '') + "\n";
            });
        }
    }
    // 7. Lab Tests
...
const getHomepageSuggestions = async (req, res) => {
    try {
        ...
        let doctors = [],  labs = [], labTests = [], labPackages = [], medicines = [], FoodServices = [];
        const tasks = [];
        ...
        if (!cleanType || cleanType === 'Foodprovider') {
            tasks.push(FoodService.find({ status: 'Approved', title: regex }).select('title pricing FoodId').populate('FoodId', 'name profileImage').limit(3).lean().then(res => FoodServices = res));
        }
        ...
        FoodServices.forEach(ns => suggestions.push({ id: ns._id, title: ns.title, subtitle: `Provided by: ${ns.FoodId?.name || "Provider"}`, type: "Food Service", image: ns.FoodId?.profileImage || null }));
...
const searchHomepage = async (req, res) => {
    try {
        ...
        let doctors = [],  labs = [], labTests = [], medicines = [], FoodServices = [];
        const tasks = [];
        ...
        if (!cleanType || cleanType === 'Foodprovider') {
            tasks.push(FoodService.find({ status: 'Approved', $or: [{ title: regex }, { description: regex }] }).populate('FoodId', 'name profileImage city speciality').limit(10).lean().then(res => FoodServices = res));
        }
        ...
        res.json({
            ...
            data: { doctors, labs, labTests, medicines, FoodServices }
        });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
