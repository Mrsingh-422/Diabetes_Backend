...
const getRefundInitiatedList = async (req, res) => {
    try {
        ...
        const [doctorAndHospital, labs,  pharmacies] = await Promise.all([
...
// 2. POST: Execute refund for a booking (With  integration added)
// Endpoint: POST /api/admin/refunds/process
const processAdminRefund = async (req, res) => {
    try {
        const { bookingId, vendorModel } = req.body; // vendorModel: 'Doctor' | 'Hospital' | 'Lab' | 'Pharmacy' | 
