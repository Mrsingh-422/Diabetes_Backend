...
// 1. GET ALL PENDING WITHDRAWALS (Admin dashboard lists all doctors, labs, Foods requests) - [1.2.2]
...
const verifyVendorBankDetails = async (req, res) => {
    try {
        ...
        switch (vendorModel) {
            ...
            case 'Food': VendorModel = Food; break;
...
const getPendingBankVerifications = async (req, res) => {
    try {
        ...
        const [doctors,  labs, pharmacies, Foods, ] = await Promise.all([
            ...
            Food.find(queryFilter).select(selectFields).lean(),
        ]);
        ...
        const pendingBanksList = [
            ...
            ...Foods.map(n => ({ vendorId: n._id, name: n.name, email: n.email, phone: n.phone, vendorModel: 'Food', bankDetails: n.bankDetails })),
...
const getAdminWalletDashboardStats = async (req, res) => {
    try {
        ...
        const [doctors, labs, pharmacies, Foods] = await Promise.all([
            ...
            Food.countDocuments(queryFilter),
        ]);

        const totalPendingBanks = doctors + labs + pharmacies + Foods ;
